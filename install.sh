...
get_version() {
    ...
    local response
    if response=$(curl -fsSL -H "User-Agent: NexusAI-Installer" "${GITHUB_API}/latest" 2>/dev/null); then
...
build_download_url() {
    ...
    case "$PLATFORM" in
        linux)
            case "$PKG_EXT" in
                deb)
                    # NexusAI.Tools_4.1.31_amd64.deb or _arm64.deb
                    DOWNLOAD_URL="${base_url}/NexusAI.Tools_${RELEASE_VERSION}_${DEB_ARCH}.deb"
                    FILENAME="NexusAI.Tools_${RELEASE_VERSION}_${DEB_ARCH}.deb"
                    ;;
                rpm)
                    # NexusAI.Tools-4.1.31-1.x86_64.rpm or -1.aarch64.rpm
                    DOWNLOAD_URL="${base_url}/NexusAI.Tools-${RELEASE_VERSION}-1.${RPM_ARCH}.rpm"
                    FILENAME="NexusAI.Tools-${RELEASE_VERSION}-1.${RPM_ARCH}.rpm"
                    ;;
                AppImage)
                    # NexusAI.Tools_4.1.31_amd64.AppImage or _aarch64.AppImage
                    local appimage_arch
                    ...
                    DOWNLOAD_URL="${base_url}/NexusAI.Tools_${RELEASE_VERSION}_${appimage_arch}.AppImage"
                    FILENAME="NexusAI.Tools_${RELEASE_VERSION}_${appimage_arch}.AppImage"
                    ;;
            esac
            ;;
        macos)
            # Prefer universal DMG, fallback to arch-specific
            DOWNLOAD_URL="${base_url}/NexusAI.Tools_${RELEASE_VERSION}_universal.dmg"
            FILENAME="NexusAI.Tools_${RELEASE_VERSION}_universal.dmg"
            ;;
    esac
...
install_linux() {
    ...
    case "$PKG_MANAGER" in
        ...
        appimage)
            ...
            run cp "$DOWNLOAD_PATH" "${install_dir}/nexusai-tools"

            if [[ ":$PATH:" != *":${install_dir}:"* ]]; then
                warn "Add ${install_dir} to your PATH to run nexusai-tools from anywhere"
