...
- [ ] Verify backup suffix: `.nexusai-manager.bak` (new) and `.nexusai.bak` (legacy)
- [ ] Verify backup location: `~/.config/opencode/opencode.json.nexusai-manager.bak`
- [ ] Verify accounts backup: `~/.config/opencode/nexusai-accounts.json.nexusai-manager.bak`
...
- [ ] `provider.nexusai-manager` created with correct structure
...
```
- [ ] File created at `~/.config/opencode/nexusai-accounts.json`
- [ ] `version` field = 3
...
```bash
opencode run "test" --model nexusai-manager/claude-sonnet-4-6-thinking --variant high
```
...
```bash
opencode run "test" --model nexusai-manager/gemini-3-pro-high --variant low
```
...
```bash
# If opencode-nexusai-auth plugin installed
opencode run "test" --model google/nexusai-claude-sonnet-4-6-thinking --variant max
```
...
### 5.1 Clear Config
- [ ] Removes `provider.nexusai-manager`
...
|-------------|-----------------|
| New suffix (`.nexusai-manager.bak`) | Restores successfully |
| Old suffix (`.nexusai.bak`) | Restores successfully (backward compatible) |
