# 🐋 NexusAI Manager 原生 Docker 部署手冊

本目錄包含 NexusAI Manager 的原生 Headless Docker 部署方案。該方案支持完整的 Web 管理界面、API 反代以及數據持久化，無需複雜的 VNC 或桌面環境。
...
> *   **查看方式**：執行 `docker logs nexusai-manager` 尋找 `Current API Key` 或 `Web UI Password`，或執行 `grep -E '"api_key"|"admin_password"' ~/.nexusai_tools/gui_config.json` 查看。
...
```bash
# 默認構建最新標籤
docker build -t nexusai-manager:latest -f docker/Dockerfile .
```
...
```bash
# 強制使用国内镜像加速構建
docker build --build-arg USE_MIRROR=true -t nexusai-manager:latest -f docker/Dockerfile .
```
...
## 📂 數據持久化
請務必將宿主機目錄掛載至容器內的 `/root/.nexusai_tools`，否則賬號和配置在容器重啟後會丟失。
...
```bash
# 打上版本標籤並推送
docker tag nexusai-manager:latest VisualLock/nexusai-manager:latest
docker tag nexusai-manager:latest VisualLock/nexusai-manager:4.1.20
docker push VisualLock/nexusai-manager:latest
docker push VisualLock/nexusai-manager:4.1.20
```
