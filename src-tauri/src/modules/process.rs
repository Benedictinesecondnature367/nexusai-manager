...
pub fn is_nexusai_running() -> bool {
    ...
    let manual_path = crate::modules::config::load_app_config()
        .ok()
        .and_then(|c| c.nexusai_executable)
    ...
    for (pid, process) in system.processes() {
        ...
        // Exclude own path (handles case where manager is mistaken for NexusAI on Linux)
        ...
        #[cfg(target_os = "macos")]
        {
            if exe_path.contains("nexusai.app") && !is_helper {
                return true;
            }
        }
        ...
        #[cfg(target_os = "linux")]
        {
            if (name.contains("nexusai") || exe_path.contains("/nexusai"))
                && !name.contains("tools")
                && !is_helper
            {
                return true;
...
/// Get PIDs of all NexusAI processes (including main and helper processes)
fn get_nexusai_pids() -> Vec<u32> {
    let mut system = System::new();
    system.refresh_processes(sysinfo::ProcessesToUpdate::All);
    ...
    for (pid, process) in system.processes() {
        ...
        #[cfg(target_os = "macos")]
        {
            // Match processes within NexusAI main app bundle, excluding Helper/Plugin/Renderer etc.
            if exe_path.contains("nexusai.app") && !is_helper {
                pids.push(pid_u32);
            }
        }
        ...
        {
            let name = process.name().to_string_lossy().to_lowercase();
            if (name == "nexusai" || exe_path.contains("/nexusai"))
                && !name.contains("tools")
                && !is_helper
            {
                pids.push(pid_u32);
...
/// Close NexusAI processes
pub fn close_nexusai(#[allow(unused_variables)] timeout_secs: u64) -> Result<(), String> {
    crate::modules::logger::log_info("Closing NexusAI...");

    #[cfg(target_os = "windows")]
    {
        // Windows: Precise kill by PID to support multiple versions or custom filenames
        let pids = get_nexusai_pids();
        if !pids.is_empty() {
            crate::modules::logger::log_info(&format!(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if !pids.is_empty() {
            ...
            while start.elapsed() < Duration::from_secs(graceful_timeout) {
                if !is_nexusai_running() {
                    crate::modules::logger::log_info("All NexusAI processes gracefully closed");
                    return Ok(());
                }
            ...
            if is_nexusai_running() {
                let remaining_pids = get_nexusai_pids();
            ...
            crate::modules::logger::log_info("NexusAI not running, no need to close");
            return Ok(());
        }
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if !pids.is_empty() {
            ...
            while start.elapsed() < Duration::from_secs(graceful_timeout) {
                if !is_nexusai_running() {
                    crate::modules::logger::log_info("NexusAI gracefully closed");
                    return Ok(());
                }
                thread::sleep(Duration::from_millis(500));
            }
            ...
            crate::modules::logger::log_info(
                "No NexusAI processes found to close (possibly filtered or not running)",
            );
        }
    }
    ...
    // Final check
    if is_nexusai_running() {
        return Err("Unable to close NexusAI process, please close manually and retry".to_string());
    }

    crate::modules::logger::log_info("NexusAI closed successfully");
    Ok(())
}

/// Start NexusAI
#[allow(unused_mut)]
pub fn start_nexusai() -> Result<(), String> {
    crate::modules::logger::log_info("Starting NexusAI...");
    ...
    let manual_path = config
        .as_ref()
        .and_then(|c| c.nexusai_executable.clone());
    let args = config.and_then(|c| c.nexusai_args.clone());
    ...
    if let Some(mut path_str) = manual_path {
        ...
        if path.exists() {
            ...
            crate::modules::logger::log_info(&format!(
                "NexusAI startup command sent (manual path: {}, args: {:?})",
                path_str, args
            ));
            return Ok(());
        } else {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if has_args {
            if let Some(detected_path) = get_nexusai_executable_path() {
                ...
                return Err("Startup arguments configured but cannot find NexusAI executable path. Please set the executable path manually in Settings.".to_string());
            }
        } else {
            use crate::utils::command::CommandExtWrapper;
            let mut cmd = Command::new("cmd");
            cmd.creation_flags_windows();
            cmd.args(["/C", "start", "nexusai://"]);
            
            let result = cmd.spawn();
            if result.is_err() {
                return Err("Startup failed, please open NexusAI manually".to_string());
            }
        }
    }
    ...
    #[cfg(target_os = "linux")]
    {
        let mut cmd = Command::new("nexusai");
    ...
    crate::modules::logger::log_info(&format!(
        "NexusAI startup command sent (default detection, args: {:?})",
        args
    ));
    Ok(())
}

/// Get NexusAI executable path and startup arguments from running processes
///
    ...
    for (pid, process) in system.processes() {
        ...
        if let Some(exe) = process.exe() {
            ...
            #[cfg(target_os = "macos")]
            {
                ...
                if exe_path.contains("nexusai.app")
                    && !is_helper
            ...
            #[cfg(target_os = "linux")]
            {
                // Linux: Check process name or path for nexusai, excluding helpers and manager
                if (name == "nexusai" || exe_path.contains("/nexusai"))
                    && !name.contains("tools")
                    && !is_helper
                {
...
/// Get NexusAI executable path from running processes
///
...
/// Get NexusAI startup arguments from running processes
...
pub fn get_user_data_dir_from_process() -> Option<std::path::PathBuf> {
    ...
    if let Ok(config) = crate::modules::config::load_app_config() {
        if let Some(args) = config.nexusai_args {
...
/// Get NexusAI executable path (cross-platform)
///
...
/// 3. Return None
pub fn get_nexusai_executable_path() -> Option<std::path::PathBuf> {
...
fn check_standard_locations() -> Option<std::path::PathBuf> {
    #[cfg(target_os = "macos")]
    {
        let path = std::path::PathBuf::from("/Applications/NexusAI.app");
        if path.exists() {
            return Some(path);
        }
    }
    ...
    #[cfg(target_os = "linux")]
    {
        let possible_paths = vec![
            std::path::PathBuf::from("/usr/bin/nexusai"),
            std::path::PathBuf::from("/opt/NexusAI/nexusai"),
            std::path::PathBuf::from("/usr/share/nexusai/nexusai"),
        ];

        // User local installation
        if let Some(home) = dirs::home_dir() {
            let user_local = home.join(".local/bin/nexusai");
