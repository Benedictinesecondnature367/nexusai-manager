...
pub fn get_storage_path() -> Result<PathBuf, String> {
    ...
    if let Some(exe_path) = process::get_nexusai_executable_path() {
    ...
    #[cfg(target_os = "macos")]
    {
        let home = dirs::home_dir().ok_or("failed_to_get_home_dir")?;
        let path =
            home.join("Library/Application Support/NexusAI/User/globalStorage/storage.json");
        if path.exists() {
            return Ok(path);
        }
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let path = PathBuf::from(appdata).join("NexusAI\\User\\globalStorage\\storage.json");
        if path.exists() {
            return Ok(path);
        }
    }
    ...
    {
        let home = dirs::home_dir().ok_or("failed_to_get_home_dir")?;
        let path = home.join(".config/NexusAI/User/globalStorage/storage.json");
        if path.exists() {
            return Ok(path);
        }
    }
