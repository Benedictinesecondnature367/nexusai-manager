...
fn get_nexusai_path() -> Option<PathBuf> {
    if let Ok(config) = crate::modules::config::load_app_config() {
        if let Some(path_str) = config.nexusai_executable {
    ...
    crate::modules::process::get_nexusai_executable_path()
}

/// Get NexusAI database path (cross-platform)
pub fn get_db_path() -> Result<PathBuf, String> {
    ...
    if let Some(nexusai_path) = get_nexusai_path() {
        if let Some(parent_dir) = nexusai_path.parent() {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        Ok(home.join("Library/Application Support/NexusAI/User/globalStorage/state.vscdb"))
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        Ok(PathBuf::from(appdata).join("NexusAI\\User\\globalStorage\\state.vscdb"))
    }

    #[cfg(target_os = "linux")]
    {
        let home = dirs::home_dir().ok_or("Failed to get home directory")?;
        Ok(home.join(".config/NexusAI/User/globalStorage/state.vscdb"))
    }
}
...
pub fn inject_token(
    ...
    // 1. Detect NexusAI version
    let version_result = crate::modules::version::get_nexusai_version();
    
    match version_result {
        Ok(ver) => {
            crate::modules::logger::log_info(&format!(
                "Detected NexusAI version: {}",
                ver.short_version
            ));
            ...
            if crate::modules::version::is_new_version(&ver) {
                ...
                crate::modules::logger::log_info(
                    "Using new format injection (nexusaiUnifiedStateSync.oauthToken)",
                );
                inject_new_format(
                    db_path,
                    access_token,
...
fn inject_new_format(
    ...
    conn.execute(
        ...
        ["nexusaiUnifiedStateSync.oauthToken", &outer_b64],
    )
...
fn inject_user_status(conn: &Connection, email: &str) -> Result<(), String> {
    ...
    conn.execute(
        "INSERT OR REPLACE INTO ItemTable (key, value) VALUES (?, ?)",
        ["nexusaiUnifiedStateSync.userStatus", &entry_b64],
    )
...
fn inject_enterprise_project_preference(conn: &Connection, project_id: &str) -> Result<(), String> {
    ...
    conn.execute(
        "INSERT OR REPLACE INTO ItemTable (key, value) VALUES (?, ?)",
        [
            "nexusaiUnifiedStateSync.enterprisePreferences",
            &entry_b64,
        ],
    )
    ...
    let current_data: String = conn
        ...
        .map_err(|e| match e {
            SqliteError::QueryReturnedNoRows => {
                "Old format key does not exist, possibly new version NexusAI".to_string()
            }
