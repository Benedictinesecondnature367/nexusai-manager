//! NexusAI cache clearing module
//!
//! Provides functionality to clear NexusAI application cache directories
...
/// Get all known NexusAI cache paths for the current platform
pub fn get_nexusai_cache_paths() -> Vec<PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        if let Some(home) = dirs::home_dir() {
            ...
            paths.push(home.join("Library/HTTPStorages/com.google.nexusai"));

            // Application caches
            paths.push(home.join("Library/Caches/com.google.nexusai"));

            // Alternative cache locations that may exist
            paths.push(home.join(".nexusai"));
            paths.push(home.join(".config/nexusai"));
        }
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if let Ok(local_app_data) = std::env::var("LOCALAPPDATA") {
            ...
            paths.push(local_path.join("Google\\NexusAI"));
            paths.push(local_path.join("NexusAI\\Cache"));
        }

        // AppData cache
        if let Ok(app_data) = std::env::var("APPDATA") {
            let app_path = PathBuf::from(&app_data);
            paths.push(app_path.join("NexusAI\\Cache"));
        }
    }
    ...
    #[cfg(target_os = "linux")]
    {
        if let Some(home) = dirs::home_dir() {
            // XDG cache directory
            paths.push(home.join(".cache/NexusAI"));
            paths.push(home.join(".cache/google-nexusai"));
        ...
        if let Ok(xdg_cache) = std::env::var("XDG_CACHE_HOME") {
            ...
            paths.push(cache_path.join("NexusAI"));
            paths.push(cache_path.join("google-nexusai"));
        }
    }
...
/// Clear NexusAI application cache
///
/// # Arguments
...
pub fn clear_nexusai_cache(custom_paths: Option<Vec<String>>) -> Result<ClearResult, String> {
    let paths: Vec<PathBuf> = match custom_paths {
        Some(custom) => custom.into_iter().map(PathBuf::from).collect(),
        None => get_nexusai_cache_paths(),
    };

    logger::log_info(&format!(
        "Starting NexusAI cache clearing, {} potential paths",
        paths.len()
    ));
    ...
    logger::log_info(&format!(
        "NexusAI cache clearing completed: {} paths cleared, {:.2} MB freed, {} errors",
...
mod tests {
    ...
    fn test_get_cache_paths_not_empty() {
        let paths = get_nexusai_cache_paths();
