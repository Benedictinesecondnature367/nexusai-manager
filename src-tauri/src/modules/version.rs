...
/// 检测 NexusAI 版本（跨平台）
pub fn get_nexusai_version() -> Result<NexusAIVersion, String> {
    // 1. 获取 NexusAI 可执行文件路径（复用现有功能）
    let exe_path = process::get_nexusai_executable_path()
        .ok_or("Unable to locate NexusAI executable")?;
...
#[cfg(target_os = "macos")]
fn get_version_macos(exe_path: &PathBuf) -> Result<NexusAIVersion, String> {
    use plist::Value;
    
    // exe_path 可能是 /Applications/NexusAI.app 或内部可执行文件
    // 需要找到 .app 目录
...
#[cfg(target_os = "windows")]
fn get_version_windows(exe_path: &PathBuf) -> Result<NexusAIVersion, String> {
...
#[cfg(target_os = "linux")]
fn get_version_linux(exe_path: &PathBuf) -> Result<NexusAIVersion, String> {
    use std::process::Command;
    ...
    Err("Unable to determine NexusAI version on Linux".to_string())
}

/// 判断是否为新版本 (>= 1.16.5)
pub fn is_new_version(version: &NexusAIVersion) -> bool {
