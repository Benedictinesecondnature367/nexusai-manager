...
// ... existing constants ...
const DATA_DIR: &str = ".nexusai_tools";
...
fn is_enterprise_client(client_key: Option<&str>) -> bool {
    client_key
        ...
        .map(|key| key.eq_ignore_ascii_case("nexusai_enterprise"))
        .unwrap_or(false)
}
...
fn format_switch_refresh_error(message: &str) -> String {
    ...
    {
        return format!(
            "Token refresh failed: OAuth client is not authorized for this account. Please sign in again in NexusAI-Manager and complete authorization/verification. Raw error: {}",
            message
        );
    }
    ...
    {
        return format!(
            "Token refresh failed: account requires additional verification. Please finish verification in NexusAI, then retry account switch. Raw error: {}",
            message
        );
    }
