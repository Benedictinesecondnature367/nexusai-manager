...
const OAUTH_CLIENTS_ENV: &str = "NEXUSAI_OAUTH_CLIENTS";
const ACTIVE_OAUTH_CLIENT_ENV: &str = "NEXUSAI_OAUTH_CLIENT_KEY";
const DEFAULT_OAUTH_CLIENT_KEY: &str = "nexusai_enterprise";
...
fn build_registry() -> OAuthClientRegistry {
    let mut clients: Vec<OAuthClientConfig> = vec![OAuthClientConfig {
        ...
        label: "NexusAI Enterprise".to_string(),
...
fn normalize_refreshed_oauth_client_key(
    ...
    if current_token.oauth_client_key.is_none()
        && project_missing
        && matches!(resolved.as_deref(), Some("nexusai_enterprise"))
    {
