...
/// 用于解决登录失败、版本验证错误等问题
#[tauri::command]
pub async fn clear_nexusai_cache() -> Result<modules::cache::ClearResult, String> {
    modules::cache::clear_nexusai_cache(None)
}

/// 获取 NexusAI 缓存路径列表（用于预览）
#[tauri::command]
pub async fn get_nexusai_cache_paths() -> Result<Vec<String>, String> {
...
/// 获取 NexusAI 可执行文件路径
#[tauri::command]
pub async fn get_nexusai_path(bypass_config: Option<bool>) -> Result<String, String> {
    // 1. 优先从配置查询 (除非明确要求绕过)
    if bypass_config != Some(true) {
        if let Ok(config) = crate::modules::config::load_app_config() {
            if let Some(path) = config.nexusai_executable {
    ...
    // 2. 执行实时探测
    match crate::modules::process::get_nexusai_executable_path() {
        Some(path) => Ok(path.to_string_lossy().to_string()),
        None => Err("未找到 NexusAI 安装路径".to_string()),
    }
}

/// 获取 NexusAI 启动参数
#[tauri::command]
pub async fn get_nexusai_args() -> Result<Vec<String>, String> {
    match crate::modules::process::get_args_from_running_process() {
        Some(args) => Ok(args),
        None => Err("未找到正在运行的 NexusAI 进程".to_string()),
    }
}
