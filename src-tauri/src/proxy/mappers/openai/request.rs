...
pub fn transform_openai_request(
    ...
    **Proactiveness**";
    ...
    let user_has_nexusai = system_instructions
        .iter()
        .any(|s| s.contains("You are NexusAI"));

    let mut parts = Vec::new();

    // 1. NexusAI 身份 (如果需要, 作为独立 Part 插入)
    if !user_has_nexusai {
        parts.push(json!({"text": nexusai_identity}));
    }

    // 2. [NEW] 注入全局系统提示词 (紧跟 NexusAI 身份之后)
    ...
    let final_body = json!({
        ...
        "requestId": format!("agent/nexusai/{}/{}", &session_id[..session_id.len().min(8)], message_count),
