...
/// 这是最后一道防线,确保发送给 NexusAI 的请求中不包含任何 cache_control。
...
pub fn transform_claude_request_in(
    ...
    let request_id = format!("agent/nexusai/{}/{}", &session_id[..session_id.len().min(8)], message_count);
    ...
    // 确保发送给 NexusAI 的请求中不包含任何 cache_control
...
fn should_enable_thinking_by_default(model: &str) -> bool {
    ...
    // unless they have "-thinking" suffix (which they don't in NexusAI mapping)
...
fn build_system_instruction(
    ...
    // [NEW] NexusAI 身份指令 (原始简化版)
    let nexusai_identity = "You are NexusAI, a powerful agentic AI coding assistant designed by the Google Deepmind team working on Advanced Agentic Coding.\n\
    You are pair programming with a USER to solve their coding task. The task may require creating a new codebase, modifying or debugging an existing codebase, or simply answering a question.\n\
    **Absolute paths only**\n\
    **Proactiveness**";

    // [HYBRID] 检查用户是否已提供 NexusAI 身份
    let mut user_has_nexusai = false;
    if let Some(sys) = system {
        match sys {
            SystemPrompt::String(text) => {
                if text.contains("You are NexusAI") {
                    user_has_nexusai = true;
                }
            }
            SystemPrompt::Array(blocks) => {
                for block in blocks {
                    if block.block_type == "text" && block.text.contains("You are NexusAI") {
                        user_has_nexusai = true;
                        break;
                    }
                }
    ...
    // 如果用户没有提供 NexusAI 身份,则注入
    if !user_has_nexusai {
        parts.push(json!({"text": nexusai_identity}));
    }

    // [NEW] 注入全局系统提示词 (紧跟 NexusAI 身份之后)
