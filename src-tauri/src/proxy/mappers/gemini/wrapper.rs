...
pub fn wrap_request(
    ...
    if let Some(image_config) = config.image_config {
        ...
        // [NEW] 只在非图像生成模式下注入 NexusAI 身份 (原始简化版)
        ...
        if let Some(system_instruction) = inner_request.get_mut("systemInstruction") {
            ...
            if let Some(parts) = system_instruction.get_mut("parts") {
                if let Some(parts_array) = parts.as_array_mut() {
                    // 检查第一个 part 是否已包含 NexusAI 身份
                    let has_nexusai = parts_array
                        .get(0)
                        .and_then(|p| p.get("text"))
                        .and_then(|t| t.as_str())
                        .map(|s| s.contains("You are NexusAI"))
                        .unwrap_or(false);

                    if !has_nexusai {
                        // 在前面插入 NexusAI 身份
                        parts_array.insert(0, json!({"text": nexusai_identity}));
                    }

                    // [NEW] 注入全局系统提示词 (紧跟 NexusAI 身份之后，用户指令之前)
                    ...
                    {
                        // 插入位置：NexusAI 身份之后 (index 1)
                        let insert_pos = if has_nexusai { 1 } else { 1 };
            ...
            let mut parts = vec![json!({"text": nexusai_identity})];
            // [NEW] 注入全局系统提示词
            let global_prompt_config = crate::proxy::config::get_global_system_prompt();
    ...
    let final_request = json!({
        ...
        "requestId": format!("agent/nexusai/{}/{}", &sid[..sid.len().min(8)], message_count),
...
mod tests {
    ...
    #[test]
    fn test_nexusai_identity_injection_with_role() {
        let body = json!({
            "model": "gemini-pro",
            "messages": []
        });
    ...
    fn test_user_instruction_preservation() {
        ...
        // Should have 2 parts: NexusAI + User
