...
impl AxumServer {
    ...
    proxy_pool_manager.clone().start_health_check_loop();
        ...
        let admin_routes = Router::new()
            ...
            .route("/system/nexusai/path", get(admin_get_nexusai_path))
            .route("/system/nexusai/args", get(admin_get_nexusai_args))
            .route("/system/cache/clear", post(admin_clear_nexusai_cache))
            .route(
                "/system/cache/paths",
                get(admin_get_nexusai_cache_paths),
            )
...
async fn admin_get_nexusai_path() -> Result<impl IntoResponse, (StatusCode, Json<ErrorResponse>)>
{
    let path = crate::commands::get_nexusai_path(Some(true))
        .await
        .map_err(|e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
...
async fn admin_get_nexusai_args() -> Result<impl IntoResponse, (StatusCode, Json<ErrorResponse>)>
{
    let args = crate::commands::get_nexusai_args().await.map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ErrorResponse { error: e }),
        )
...
async fn admin_clear_nexusai_cache(
) -> Result<impl IntoResponse, (StatusCode, Json<ErrorResponse>)> {
    let res = crate::commands::clear_nexusai_cache().await.map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ErrorResponse { error: e }),
        )
...
async fn admin_get_nexusai_cache_paths(
) -> Result<impl IntoResponse, (StatusCode, Json<ErrorResponse>)> {
    let res = crate::commands::get_nexusai_cache_paths()
        .await
        .map_err(|e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
