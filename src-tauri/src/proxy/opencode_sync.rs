...
const NEXUSAI_CONFIG_FILE: &str = "nexusai.json";
const NEXUSAI_ACCOUNTS_FILE: &str = "nexusai-accounts.json";
const BACKUP_SUFFIX: &str = ".nexusai-manager.bak";
const OLD_BACKUP_SUFFIX: &str = ".nexusai.bak";

const NEXUSAI_PROVIDER_ID: &str = "nexusai-manager";
...
/// Build the complete model catalog for nexusai-manager provider
...
fn get_config_paths() -> Option<(PathBuf, PathBuf, PathBuf)> {
    get_opencode_dir().map(|dir| {
        (
            ...
            dir.join(NEXUSAI_CONFIG_FILE),
            dir.join(NEXUSAI_ACCOUNTS_FILE),
        )
    })
}
...
pub fn get_sync_status(proxy_url: &str) -> (bool, bool, Option<String>) {
    ...
    // Only check nexusai-manager provider
    let ag_opts = get_provider_options(&json, NEXUSAI_PROVIDER_ID);
    let ag_url = ag_opts
...
pub fn restore_opencode_config() -> Result<(), String> {
    ...
    let accounts_backup_new = accounts_path.with_file_name(format!(
        "{}{}", NEXUSAI_ACCOUNTS_FILE, BACKUP_SUFFIX
    ));
    let accounts_backup_old = accounts_path.with_file_name(format!(
        "{}{}", NEXUSAI_ACCOUNTS_FILE, OLD_BACKUP_SUFFIX
    ));
...
fn apply_sync_to_config(
    ...
    if let Some(provider) = config.get_mut("provider").and_then(|p| p.as_object_mut()) {
        ensure_provider_object(provider, NEXUSAI_PROVIDER_ID);
        if let Some(ag_provider) = provider.get_mut(NEXUSAI_PROVIDER_ID) {
            ensure_provider_string_field(ag_provider, "npm", "@ai-sdk/anthropic");
            ensure_provider_string_field(ag_provider, "name", "NexusAI Manager");
...
fn apply_clear_to_config(
    ...
    if let Some(provider) = config.get_mut("provider").and_then(|p| p.as_object_mut()) {
        // 1. Remove nexusai-manager provider
        provider.remove(NEXUSAI_PROVIDER_ID);
...
mod tests {
    ...
    #[test]
    fn test_sync_creates_nexusai_provider() {
        let config = serde_json::json!({});

        let result = apply_sync_to_config(config, "http://localhost:3000", "test-api-key", None);

        // nexusai-manager provider should be created
        let provider = result.get("provider").unwrap();
        let ag = provider.get(NEXUSAI_PROVIDER_ID).unwrap();

        // Check npm and name
        assert_eq!(ag.get("npm").unwrap(), "@ai-sdk/anthropic");
        assert_eq!(ag.get("name").unwrap(), "NexusAI Manager");
    ...
    #[test]
    fn test_clear_removes_nexusai_provider() {
        let config = serde_json::json!({
            "provider": {
                "nexusai-manager": {
        ...
        let provider = result.get("provider").unwrap();
        assert!(provider.get(NEXUSAI_PROVIDER_ID).is_none(), "nexusai-manager should be removed");
        assert!(provider.get("google").is_some(), "google should be preserved");
    }

    #[test]
    fn test_clear_legacy_removes_nexusai_models() {
        let config = serde_json::json!({
            "provider": {
                "anthropic": {
        ...
        // NexusAI model IDs should be removed
        assert!(!models.contains_key("claude-sonnet-4-5"), "nexusai model should be removed");
        // Non-nexusai models should be preserved
        assert!(models.contains_key("claude-3"), "non-nexusai model should be preserved");
    }
...
pub fn read_opencode_config_content(file_name: Option<String>) -> Result<String, String> {
    ...
    let allowed_files = [
        ...
        NEXUSAI_CONFIG_FILE,
        NEXUSAI_ACCOUNTS_FILE,
    ];

    // Determine which file to read
    let target_path = match file_name.as_deref() {
        Some(name) if name == NEXUSAI_CONFIG_FILE => ag_config_path,
        Some(name) if name == NEXUSAI_ACCOUNTS_FILE => ag_accounts_path,
...
pub async fn get_opencode_sync_status(proxy_url: String) -> Result<OpencodeStatus, String> {
    ...
    Ok(OpencodeStatus {
        ...
        files: vec![
            ...
            NEXUSAI_CONFIG_FILE.to_string(),
            NEXUSAI_ACCOUNTS_FILE.to_string(),
        ],
    })
}
...
/// List of NexusAI model IDs that may have been added to legacy providers
const NEXUSAI_MODEL_IDS: &[&str] = &[
    "claude-sonnet-4-6",
...
/// Clear OpenCode config by removing nexusai-manager provider and optionally cleaning up legacy entries
