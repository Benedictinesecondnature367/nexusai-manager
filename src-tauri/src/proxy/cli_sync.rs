...
pub fn get_sync_status(app: &CliApp, proxy_url: &str) -> (bool, bool, Option<String>) {
    ...
    for file in &files {
        // 使用更简单的命名规则: original_name + .nexusai.bak
        let backup_path = file.path.with_file_name(format!("{}.nexusai.bak", file.name));
...
pub fn sync_config(app: &CliApp, proxy_url: &str, api_key: &str, model: Option<&str>) -> Result<(), String> {
    ...
    for file in &files {
        ...
        // [New Feature] 自动备份：如果文件存在且没有备份，创建 .nexusai.bak 备份
