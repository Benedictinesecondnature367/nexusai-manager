...
/// URL to fetch the latest NexusAI version
const VERSION_URL: &str = "https://nexusai-auto-updater-974169037036.us-central1.run.app";

/// Second fallback: Official Changelog page
const CHANGELOG_URL: &str = "https://nexusai.google/changelog";



/// Known stable configuration (for Docker/Headless fallback)
/// NexusAI 4.1.31 uses Electron 39.2.3 which corresponds to Chrome 132.0.6834.160
...
/// Try to fetch the latest NexusAI version from the remote update server.
...
///   - The local NexusAI install is outdated, OR
...
fn resolve_version_config() -> (VersionConfig, VersionSource) {
    ...
    if let Ok(local_ver) = crate::modules::version::get_nexusai_version() {
        ...
        if let Some(local_v) = local_parsed {
            if compare_semver(&local_v, &best_version) > std::cmp::Ordering::Equal {
                ...
                tracing::info!(
                    ...
                    "Local NexusAI version is older than known-stable floor; \
...
/// Current resolved NexusAI version (e.g., "4.1.31")
...
pub static NATIVE_OAUTH_USER_AGENT: LazyLock<String> = LazyLock::new(|| {
    format!("vscode/1.X.X (NexusAI/{})", CURRENT_VERSION.as_str())
});
...
/// "NexusAI/4.1.31 (Macintosh; Intel Mac OS X 10_15_7) Chrome/132.0.6834.160 Electron/39.2.3"
pub fn get_default_user_agent() -> String {
    format!("NexusAI/{} (Macintosh; Intel Mac OS X 10_15_7) Chrome/132.0.6834.160 Electron/39.2.3", env!("CARGO_PKG_VERSION"))
}
...
mod tests {
    ...
    fn test_parse_version_with_suffix() {
        ...
        let text = "nexusai/1.15.8 windows/amd64";
        assert_eq!(parse_version(text), Some("1.15.8".to_string()));
    }
