...
fn should_enable_tray() -> bool {
    if env_flag_enabled("NEXUSAI_DISABLE_TRAY") {
        info!("Tray disabled by NEXUSAI_DISABLE_TRAY");
        return false;
    }

    #[cfg(target_os = "linux")]
    {
        if is_wayland_session() && !env_flag_enabled("NEXUSAI_FORCE_TRAY") {
            warn!(
                "Linux Wayland session detected; disabling tray by default to avoid GTK/AppIndicator crashes. Set NEXUSAI_FORCE_TRAY=1 to force-enable."
            );
            return false;
        }
    }
...
fn configure_linux_gdk_backend() {
    ...
    let has_x11_display = std::env::var("DISPLAY")
        .map(|v| !v.trim().is_empty())
        .unwrap_or(false);
    let force_wayland = env_flag_enabled("NEXUSAI_FORCE_WAYLAND");
    let force_x11 = env_flag_enabled("NEXUSAI_FORCE_X11");
    ...
    if force_x11 || (is_wayland && has_x11_display && !force_wayland) {
        ...
        warn!(
            "Forcing GDK_BACKEND=x11 for stability on Wayland. Set NEXUSAI_FORCE_WAYLAND=1 to keep Wayland backend."
        );
    }
}
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::clear_nexusai_cache,
            commands::get_nexusai_cache_paths,
            commands::open_data_folder,
            commands::get_data_dir_path,
            commands::show_main_window,
            commands::set_window_theme,
            commands::get_nexusai_path,
            commands::get_nexusai_args,
