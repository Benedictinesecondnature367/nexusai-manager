...
export function OpenCodeSyncModal({ proxyUrl, apiKey, onClose, onSyncDone }: OpenCodeSyncModalProps) {
    ...
    const { models: nexusaiModels } = useProxyModels();
    ...
    const rebuildPreview = useCallback((selectedIds: Set<string>) => {
        const selected = nexusaiModels.filter(m => selectedIds.has(m.id));
    ...
    if (!configLoaded) {
        ...
        invoke<string>('get_opencode_config_content', { request: { fileName: 'opencode.json' } })
            .then(content => {
                ...
                // Priority 1: Read from nexusai-manager provider
                if (parsed.provider?.['nexusai-manager']?.models) {
                    Object.keys(parsed.provider['nexusai-manager'].models).forEach(k => existingModelIds.add(k));
                }
                ...
                const hasAuth = plugins.some((p: string) => p.includes('opencode-nexusai-auth'));
                setHasAuthPlugin(hasAuth);

                // Try to extract existing baseURL from nexusai-manager provider
                if (parsed.provider?.['nexusai-manager']?.options?.baseURL) {
                    setCustomBaseUrl(parsed.provider['nexusai-manager'].options.baseURL);
                }
    ...
    const allSelected = nexusaiModels.length > 0 && nexusaiModels.every(m => selectedModels.has(m.id));
    const toggleAll = () => {
        const next = allSelected ? new Set<string>() : new Set(nexusaiModels.map(m => m.id));
    ...
    const groups = [...new Set(nexusaiModels.map(m => m.group))];
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <div className="relative group">
                            <input
                                ...
                                placeholder="e.g. http://nexusai-manager:8045/v1"
                ...
                <div className="px-5 pb-3 shrink-0 border-b border-gray-100 dark:border-base-200">
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                            ...
                            <span className="ml-2 text-gray-300">{selectedModels.size}/{nexusaiModels.length}</span>
                        </span>
                    ...
                    <div className="space-y-2 max-h-[25vh] overflow-auto">
                        {groups.map(group => {
                            const groupModels = nexusaiModels.filter(m => m.group === group);
                            return (
                                <div key={group}>
                    ...
                    <div className="px-5 py-2 shrink-0 bg-amber-50 dark:bg-amber-900/20 border-y border-amber-100 dark:border-amber-900/30">
                        <p className="text-[10px] text-amber-700 dark:text-amber-400 leading-relaxed">
                            {t('proxy.config.opencode_sync.auth_plugin_warning', {
                                defaultValue: 'Sync chỉ tạo provider nexusai-manager và không ghi đè google provider/plugin.'
                            })}
                        </p>
                    </div>
                )}
