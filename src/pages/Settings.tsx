...
function Settings() {
    ...
    const [dataDirPath, setDataDirPath] = useState<string>('~/.nexusai_tools/');

    // NexusAI cache clearing state
    ...
    const handleSelectNexusAIPath = async () => {
        try {
            const selected = await open({
                directory: false,
                multiple: false,
                title: t('settings.advanced.nexusai_path_select'),
            });
            if (selected && typeof selected === 'string') {
                setFormData({ ...formData, nexusai_executable: selected });
            }
        } catch (error) {
            showToast(`${t('common.error')}: ${error}`, 'error');
        }
    ...
    const handleDetectNexusAIPath = async () => {
        try {
            const command = isTauri() ? 'get_nexusai_path' : 'get_nexusai_path'; // 后端已统一
            const path = await invoke<string>(command, { bypassConfig: true });
            setFormData({ ...formData, nexusai_executable: path });
            showToast(t('settings.advanced.nexusai_path_detected'), 'success');
        } catch (error) {
            showToast(`${t('common.error')}: ${error}`, 'error');
        }
    };
    ...
    const handleOpenClearCacheDialog = async () => {
        try {
            const paths = await invoke<string[]>('get_nexusai_cache_paths');
            setCachePaths(paths);
    ...
    // Handle clearing NexusAI cache
    const confirmClearNexusAICache = async () => {
        ...
        <div className="h-full w-full overflow-y-auto">
            <div className="p-5 space-y-4 max-w-7xl mx-auto">
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-900 dark:text-base-content mb-1">
                                        {t('settings.advanced.nexusai_path')}
                                    </label>
                                    <div className="flex gap-2">
                                        <input
                                            type="text"
                                            className="flex-1 px-4 py-4 border border-gray-200 dark:border-base-300 rounded-lg bg-gray-50 dark:bg-base-200 text-gray-900 dark:text-base-content font-medium"
                                            value={formData.nexusai_executable || ''}
                                            placeholder={t('settings.advanced.nexusai_path_placeholder')}
                                            onChange={(e) => setFormData({ ...formData, nexusai_executable: e.target.value })}
                                        />
                                        {formData.nexusai_executable && (
                                            <button
                                                className="px-4 py-2 border border-gray-200 dark:border-base-300 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors"
                                                onClick={() => setFormData({ ...formData, nexusai_executable: undefined })}
                                            >
                                                {t('common.clear')}
                                            </button>
                                        )}
                                        <button
                                            ...
                                            onClick={handleDetectNexusAIPath}
                                        >
                                        ...
                                        {isTauri() ? (
                                            <button
                                                className="px-4 py-2 border border-gray-200 dark:border-base-300 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-base-200 transition-colors"
                                                onClick={handleSelectNexusAIPath}
                                            >
                                                {t('settings.advanced.select_btn')}
                                            </button>
                                        ) : (
                                    ...
                                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                                        {t('settings.advanced.nexusai_path_desc')}
                                    </p>
                                </div>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-900 dark:text-base-content mb-1">
                                        {t('settings.advanced.nexusai_args')}
                                    </label>
                                    <div className="flex gap-2">
                                        <input
                                            type="text"
                                            className="flex-1 px-4 py-4 border border-gray-200 dark:border-base-300 rounded-lg bg-gray-50 dark:bg-base-200 text-gray-900 dark:text-base-content font-medium"
                                            value={formData.nexusai_args ? formData.nexusai_args.join(' ') : ''}
                                            placeholder={t('settings.advanced.nexusai_args_placeholder')}
                                            onChange={(e) => {
                                                const args = e.target.value.trim() === '' ? [] : e.target.value.split(' ').map(arg => arg.trim()).filter(arg => arg !== '');
                                                setFormData({ ...formData, nexusai_args: args });
                                            }}
                                        />
                                        <button
                                            ...
                                            onClick={async () => {
                                                try {
                                                    const args = await invoke<string[]>('get_nexusai_args');
                                                    setFormData({ ...formData, nexusai_args: args });
                                                    showToast(t('settings.advanced.nexusai_args_detected'), 'success');
                                                } catch (error) {
                                                    showToast(`${t('settings.advanced.nexusai_args_detect_error')}: ${error}`, 'error');
                                                }
                                            }}
                                        >
                                    ...
                                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                                        {t('settings.advanced.nexusai_args_desc')}
                                    </p>
                                </div>
                                ...
                                {/* NexusAI 缓存清理 */}
                                <div className="border-t border-gray-200 dark:border-base-200 pt-4">
                                    <h3 className="font-medium text-gray-900 dark:text-base-content mb-3">{t('settings.advanced.nexusai_cache_title')}</h3>
                                    <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/30 rounded-lg p-3 mb-3">
                                        <p className="text-sm text-amber-700 dark:text-amber-400">{t('settings.advanced.nexusai_cache_warning')}</p>
                                    </div>
                                    <div className="bg-gray-50 dark:bg-base-200 border border-gray-200 dark:border-base-300 rounded-lg p-3 mb-3">
                                        <p className="text-sm text-gray-600 dark:text-gray-400">{t('settings.advanced.nexusai_cache_desc')}</p>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <button
                                        ...
                                        >
                                            {t('settings.advanced.clear_nexusai_cache')}
                                        </button>
                                    </div>
                                </div>
                        ...
                        <div className="flex flex-col h-full animate-in fade-in duration-500">
                            <div className="flex-1 flex flex-col justify-center items-center space-y-8">
                                ...
                                <div className="text-center space-y-4">
                                    ...
                                    <div>
                                        <h3 className="text-3xl font-black text-gray-900 dark:text-base-content tracking-tight mb-2">{t('common.app_name', 'NexusAI Tools')}</h3>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 w-full max-w-6xl px-4">
                                    ...
                                    {/* Telegram Card */}
                                    <a
                                        href="https://t.me/NexusAIManager"
                                        target="_blank"
                                        rel="noreferrer"
                                    ...
                                    {/* GitHub Card */}
                                    <a
                                        href="https://github.com/VisualLock/NexusAI-Manager"
                                        target="_blank"
                                        rel="noreferrer"
                ...
                <ModalDialog
                    ...
                    cancelText={t('common.cancel')}
                    isDestructive={true}
                    onConfirm={confirmClearNexusAICache}
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/30 rounded-lg p-2">
                            <p className="text-xs text-amber-700 dark:text-amber-400">
                                {t('settings.advanced.nexusai_cache_warning')}
                            </p>
                        </div>
                    </div>
                </ModalDialog>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="bg-gray-50 dark:bg-base-200 rounded-lg p-3">
                            <div className="flex items-center justify-between gap-2">
                                <code className="text-xs text-gray-700 dark:text-gray-300 break-all">brew upgrade --cask nexusai-tools</code>
                                <button
                                    ...
                                    onClick={() => {
                                        navigator.clipboard.writeText('brew upgrade --cask nexusai-tools');
                        ...
                        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/30 rounded-lg p-3">
                            ...
                            <div className="flex items-center justify-between gap-2">
                                <code className="text-xs text-amber-800 dark:text-amber-300 break-all">sudo xattr -rd com.apple.quarantine "/Applications/NexusAI Tools.app"</code>
                                <button
                                    ...
                                    onClick={() => {
                                        navigator.clipboard.writeText('sudo xattr -rd com.apple.quarantine "/Applications/NexusAI Tools.app"');
                                        showToast(t('common.copied', 'Copied'), 'success');
                                    }}
                                >
                                    {t('common.copy', 'Copy')}
