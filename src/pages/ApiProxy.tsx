...
export default function ApiProxy() {
    ...
    const loadCustomPresets = () => {
        try {
            const saved = localStorage.getItem('nexusai_custom_presets');
            if (saved) {
    ...
    const saveCustomPresetsToStorage = (presets: CustomPreset[]) => {
        try {
            localStorage.setItem('nexusai_custom_presets', JSON.stringify(presets));
...
# 注意: NexusAI 支持使用 Anthropic SDK 调用任意模型
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            if (enabled) {
                                                ...
                                                const restoredValue = appConfig.proxy.saved_user_agent || 'nexusai/1.15.8 darwin/arm64';
                                                updateProxyConfig({
