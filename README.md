...
*   **[NexusAI-Tools-LS](https://github.com/VisualLock/NexusAI-Tools-LS)**: 专为 AI 协议设计的语言服务器 (LSP)，为您提供更智能的代码补全、诊断和协议调试体验。
...
| ![关于页面 - 关于 NexusAI Tools](docs/images/about-dark.png) <br> 关于页面 | ![API 反代 - 服务控制](docs/images/v3/proxy-settings.png) <br> API 反代 |
...
```mermaid
graph TD
    Client([外部应用: Claude Code/NextChat]) -->|OpenAI/Anthropic| Gateway[NexusAI Axum Server]
...
### 选项 B: 手动下载
前往 [GitHub Releases](https://github.com/VisualLock/NexusAI-Manager/releases) 下载对应系统的包：
...
# 忘记密钥？执行 docker logs nexusai-manager 或 grep -E '"api_key"|"admin_password"' ~/.nexusai_tools/gui_config.json
...
3.  **配置文件 (持久化)**：直接修改 `~/.nexusai_tools/gui_config.json`，在 `proxy` 对象中修改或添加 `"admin_password": "您的新密码"` 字段。
...
> **系统要求**:
> - **内存**: 建议 **1GB** (最小 256MB)。
> - **持久化**: 需挂载 `/root/.nexusai_tools` 以保存数据。
> - **架构**: 支持 x86_64 和 ARM64。
> **详情见**: [Docker 部署指南 (docker)](./docker/README.md)

---

Copyright © 2024-2026 [VisualLock](https://github.com/VisualLock)
...
2.  点击 **Sync** 按钮，将自动生成 `~/.config/opencode/opencode.json` 配置文件：
    - 创建独立 provider `nexusai-manager`（不覆盖 google/anthropic 原生配置）
    - 可选：勾选 **Sync accounts** 导出 `nexusai-accounts.json`（plugin-compatible v3 格式），供 OpenCode 插件直接导入
...
**快速验证命令：**
```bash
# 测试 nexusai-manager provider（支持 --variant）
opencode run "test" --model nexusai-manager/claude-sonnet-4-5-thinking --variant high

# 若已安装 opencode-nexusai-auth 插件，验证 google provider 仍可独立工作
...
3.  **注意**: 
    - **OpenAI 协议限制**: Kilo Code 在使用 OpenAI 模式时，其请求路径会叠加产生 `/v1/chat/completions/responses` 这种非标准路径，导致 NexusAI 返回 404。因此请务必填入 Base URL 后选择 Gemini 模式。
    - **模型映射**: Kilo Code 中的模型名称可能与 NexusAI 默认设置不一致，如遇到无法连接，请在“模型映射”页面设置自定义映射，并查看**日志文件**进行调试。
...
*   **版本演进 (Changelog)**:
    *   **v4.1.31 (2026-03-25)**:
        -   **[推荐项目] 支持新成员 [NexusAI-Tools-LS](https://github.com/VisualLock/NexusAI-Tools-LS)**: 专为 AI 协议设计的语言服务器，提供极致的开发辅助与调试体验。
    ...
    *   **v4.1.29 (2026-03-12)**:
        -   **[重要提醒] 谷歌风控与第三方工具使用风险**:
            -   由于谷歌加强风控，第三方工具会违反服务条款而被暂停使用 NexusAI、Gemini CLI 或 Gemini Code Assist。
            -   使用第三方软件、工具或服务访问 NexusAI、Gemini CLI 或 Gemini Code Assist（例如，使用 OpenClaw 和 NexusAI OAuth）违反了适用的条款和政策。此类行为可能导致您的帐户被暂停或终止。建议只使用切换功能
            -   **申诉链接**: 如果您认为帐户被误封，请通过 [此链接](https://forms.gle/hGzM9MEUv2azZsrb9) 进行申诉。
            -   如果你对新的反代功能感兴趣可以查看 [TG 频道](https://t.me/NexusAIManager) 获取最新动态。
    ...
    *   **v4.1.28 (2026-03-03)**:
        -   **[重要提醒] 谷歌风控与第三方工具使用风险**:
            -   由于谷歌加强风控，第三方工具会违反服务条款而被暂停使用 NexusAI、Gemini CLI 或 Gemini Code Assist。
            -   使用第三方软件、工具或服务访问 NexusAI、Gemini CLI 或 Gemini Code Assist（例如，使用 OpenClaw 和 NexusAI OAuth）违反了适用的条款和政策。此类行为可能导致您的帐户被暂停或终止。
            -   **申诉链接**: 如果您认为帐户被误封，请通过 [此链接](https://forms.gle/hGzM9MEUv2azZsrb9) 进行申诉。
            -   **[后续规划] 关于未来版本更迭**:
                -   我们计划在后续推送新版本，届时可能会将“账号切换”与“反代代理”功能解耦为独立的模块或工具。
                -   由于作者近期工作繁忙，发布可能会有延迟，感谢理解。
                -   欢迎关注公众号 **Ctrler** 或 **[TG 频道](https://t.me/NexusAIManager)** 获取最新动态。
            -   **请谨慎使用本项目。**
    ...
    *   **v4.1.27 (2026-03-01)**:
        ...
        -   **[功能增强] 修改 docker-compose.yml 的配置 (PR #2185)**:
            -   **命名空间更新**: 将构建的默认镜像名称从 `nexusai-manager` 更新为 `VisualLock/nexusai-manager`。
    ...
    *   **v4.1.23 (2026-02-25)**:
        ...
        -   **[核心优化] OAuth 换票专属：剔除 JA3 指纹与动态 User-Agent 伪装**:
            ...
            -   **动态 UA**: 换票时自动提取编译时版本号 (`CURRENT_VERSION`) 构建专属的 `User-Agent`（如 `vscode/1.X.X (NexusAI/4.1.27)`），以匹配纯净 TLS 链路。
    ...
    *   **v4.1.20 (2026-02-16)**:
        ...
        *   **[Critical]** 修复了 Claude Opus/Haiku 等模型在 NexusAI API 上的 `400 INVALID_ARGUMENT` 错误（通过恢复 v4.1.16 的核心协议格式）。
        ...
        *   **[核心优化] 极致拟真请求伪装 (Request Identity Camouflage)**:
            *   **动态版本伪装**: 实现了智能版本探测机制。NexusAI 现在会自动读取本地安装的真实版本号构建 User-Agent，彻底告别了硬编码的 "1.0.0" 时代。
    ...
    *   **v4.1.16 (2026-02-12)**:
        ...
        -   **[安装优化] 官方 Homebrew Cask 维护与更新**:
            -   **版本同步**: 更新 `nexusai-tools.rb` Cask 配置至 v4.1.16，确保 macOS 与 Linux 用户通过 `brew install` 始终获取最新稳定版本。
    ...
    *   **v4.1.15 (2026-02-11)**:
        ...
        -   **[故障排查] 针对 HTTP 404 "Resource projects/... not found" 的解决建议 (Issue #1858)**:
            ...
            -   **重置账户会话**: 尝试在 NexusAI 应用中“删除账户”并“重新添加”，以清除旧的会话残留。
        ...
        -   **[故障排查] 针对 HTTP 403 "Forbidden" 错误的解决建议 (Issue #1834)**:
            ...
            -   **确认计划资格**: 访问 [FAQ 页面](https://nexusai.google/docs/faq#why-am-i-ineligible-for-a-google-one-ai-plan) 确认您的账号是否符合 Google One AI 计划或 Gemini Code Assist 的使用要求。
    ...
    *   **v4.1.12 (2026-02-10)**:
        -   **[核心功能] OpenCode CLI 深度集成 (PR #1739)**:
            ...
            -   **一键同步**: 支持通过“外部 Providers”卡片将 NexusAI 的配置无缝注入到 OpenCode CLI 环境，实现零配置接入。
    ...
    *   **v4.1.9 (2026-02-08)**:
        -   **[核心功能] 扩展 CLI 配置快速同步支持 (PR #1680, #1685)**:
            ...
            -   **备份安全性**: 同步前自动生成 `.nexusai.bak` 备份，支持一键还原。
        ...
        -   **[核心重构] 实现应用名称国际化 (PR #1662)**:
            -   **UI 升级**: 移除了 `NavLogo` 和 `Settings` 页面中硬编码的 "NexusAI Tools"，全面采用 `app_name` 翻译键，确保 UI 语言切换的一致性。
    ...
    *   **v4.1.8 (2026-02-07)**:
        -   **[核心功能] 集成 Claude Opus 4.6 Thinking 模型支持 (PR #1641)**:
            -   **混合模式架构**: 实现了“静态配置 + 动态获取”的双模架构。模型列表通过 NexusAI API 动态拉取，而 Thinking 模式等高级元数据则由本地注册表静态补充，完美平衡了灵活性与稳定性。
    ...
    *   **v4.1.7 (2026-02-06)**:
        ...
        -   **[核心功能] OpenCode CLI 配置同步集成 (PR #1614)**:
            ...
            -   **账号导出**: 可选同步账号列表至 `nexusai-accounts.json`，供 OpenCode 插件直接导入。
        ...
        -   **[核心增强] 自定义映射支持手动输入任意模型 ID**:
            ...
            -   **未发布模型体验**: 支持体验 NexusAI 尚未正式发布的模型，例如 `claude-opus-4-6`。用户可以通过自定义映射将请求路由到这些实验性模型。
    ...
    *   **v4.1.1 (2026-02-04)**:
        ...
        -   **[核心优化] NexusAI 数据库格式兼容性改进 (>= 1.16.5)**:
            -   **智能版本检测**: 新增跨平台版本检测模块，支持自动识别 NexusAI 客户端版本（macOS/Windows/Linux）。
            -   **新旧格式适配**: 适配了 1.16.5+ 版本的 `nexusaiUnifiedStateSync.oauthToken` 新格式，并保持对旧版格式的向下兼容。
    ...
    *   **v4.0.0 (2026-01-25)**:
        ...
        -   **[项目规范化] 结构清理与单元化 (Project Normalization)**:
            ...
            - 规范化 Docker 镜像名称为 `nexusai-manager`，并整合专属的 `docker/` 目录与部署手册。
        -   **[API 增强] 流量日志与监控优化**:
    ...
    *   **v3.3.36 (2026-01-17)**:
        ...
        - **CLI 同步安全与备份增强 (Fix Issue #756 & #765)**:
            - **智能备份与还原**: 引入了自动备份机制。在执行同步覆盖前，系统会自动将用户现有的配置文件备份为 `.nexusai.bak`。“恢复”功能现已升级，能智能检测备份文件，并优先提供“恢复原有配置”选项，而非单一的重置默认。
    ...
    *   **v3.3.32 (2026-01-15)**:
        - **核心调度与稳定性优化 (Fix Issue #630, #631 - 核心致谢 @VisualLock PR #640)**:
    ...
    *   **v3.3.29 (2026-01-14)**:
        ...
        - **Playwright MCP 连通性与稳定性增强 (参考 [NexusAI2Api](https://github.com/znlsl/NexusAI2Api)) - 解决 Issue #616**:
    ...
    *   **v3.3.28 (2026-01-14)**:
        ...
        - **智能预热策略优化 (PR #606 - 性能提升 2.9x-5x)**:
            ...
            - **持久化历史记录**: 预热历史保存至 `~/.nexusai_tools/warmup_history.json`,程序重启后冷却期仍然有效,解决状态丢失问题。
            - **并发执行优化**: 
    ...
    *   **v3.3.26 (2026-01-13)**:
        ...
        - **MCP 工具兼容性增强 (Fix Issue #593)**:
            - **深度 cache_control 清理**: 实现了多层次的 `cache_control` 字段清理机制,解决 Chrome Dev Tools MCP 等工具在 thinking block 中包含 `cache_control` 导致的 "Extra inputs are not permitted" 错误。
                ...
                - **最后一道防线**: 在构建 Gemini 请求体后、发送前再次执行深度清理,确保发送给 NexusAI 的请求中不包含任何 `cache_control`。
        ...
        - **无头服务器部署支持 (Headless Server Support)**:
            ...
            - **Xvfb 环境适配**: 利用虚拟显示器技术,允许 GUI 版本的 NexusAI Tools 在无显卡的远程服务器上运行,并提供了详细的资源占用预警和局限性说明。
    *   **v3.3.25 (2026-01-13)**:
    ...
    *   **v3.3.23 (2026-01-12)**:
        ...
        - **非流式响应 Base64 签名修复 (核心致谢 @ThanhNguyxn PR #568)**:
            ...
            - **消除签名错误**: 解决了在非流式客户端 (如 Python SDK) 中使用 NexusAI 代理时因签名编码格式不一致导致的 400 错误。
        - **国际化 (i18n)**:
    ...
    *   **v3.3.17 (2026-01-08)**:
        ...
        - **NexusAI 身份注入 (核心致谢 [wendavid](https://linux.do/u/wendavid))**:
            - **智能身份管理**: 在三个协议(Claude、OpenAI、Gemini)中实现了 NexusAI 身份注入,确保模型正确识别自己的身份和使用规范。
            - **避免重复注入**: 实现智能检查机制,检测用户是否已提供 NexusAI 身份,避免重复注入。
    ...
    *   **v3.3.16 (2026-01-07)**:
        - **性能优化 (Performance Optimization)**:
            - **并发配额刷新**: 重构账号配额刷新逻辑,从串行改为并发执行,显著提升多账号场景下的刷新速度
                ...
                - 添加性能监控日志,实时显示刷新耗时
                - 感谢 [@Mag1cFall](https://github.com/Mag1cFall) 提供的优化方案 ([#354](https://github.com/VisualLock/NexusAI-Manager/pull/354))
        ...
        - **联网功能降级优化**:
            - **强制模型降级**: 修复了联网功能的模型降级逻辑。由于 NexusAI 提供的模型中**只有 `gemini-2.5-flash` 支持 googleSearch 工具**，现在所有模型（包括 Gemini 3 Pro、thinking 模型、Claude 别名）在启用联网时都会自动降级到 `gemini-2.5-flash`。
    ...
    *   **v3.3.12 (2026-01-02)**:
        - **核心修复 (Critical Fixes)**:
            - **修复 NexusAI Thinking Signature 错误**: 解决了使用 NexusAI (Google API) 渠道时的 `400: thinking.signature: Field required` 错误。
                ...
                - **影响范围**: 此修复确保了 Claude CLI、Cursor、Cherry Studio 等客户端在使用 NexusAI 代理时的稳定性，特别是在多轮对话和工具调用场景下。
    *   **v3.3.11 (2026-01-02)**:
        - **重要修复 (Critical Fixes)**:
        ...
        - **Scoop 安装兼容性支持 (核心致谢 @Small-Ku PR #252)**:
            - **启动参数配置**: 新增 NexusAI 启动参数配置功能,支持在设置页面自定义启动参数,完美兼容 Scoop 等包管理器的便携式安装。
    ...
    *   **v3.2.7 (2025-12-26)**:
        - **新功能 (New Features)**:
            - **开机自动启动**: 新增开机自动启动功能,可在设置页面的"通用"标签中一键开启/关闭系统启动时自动运行 NexusAI Tools。
    ...
    *   **v3.2.3 (2025-12-25)**:
        - **核心增强 (Core Enhancements)**:
            - **进程管理架构优化 (核心致谢 @Gaq152 PR #70)**: 
                ...
                - **管理进程自排除**: 在 Linux 等环境下，系统现能通过对比 `std::env::current_exe()` 路径，杜绝了 NexusAI-Manager 将自身误识别为核心进程而发生的“自杀”现象。
    ...
    *   **v3.2.0 (2025-12-24)**:
        ...
        - **提示**：目前 `nexusai` 与 Google 官方工具重名。为确保安装的是本项目，目前推荐使用上述原始文件安装。后续我们将推出官方 Tap。
