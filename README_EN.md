...
<div align="center">
  ...
  <p>
    <a href="https://github.com/VisualLock/NexusAI-Manager">
...
**NexusAI Tools** is an all-in-one desktop application designed for developers and AI enthusiasts. It perfectly combines multi-account management, protocol conversion, and smart request scheduling to provide you with a stable, high-speed, and low-cost **Local AI Relay Station**.
...
| <img src="docs/images/AICodeMirror.jpg" width="200" alt="AICodeMirror Logo"> | Thanks to **AICodeMirror** for sponsoring this project! AICodeMirror provides official high-stability relay services for Claude Code / Codex / Gemini CLI, supporting enterprise-grade concurrency, fast invoicing, and 24/7 dedicated technical support. Claude Code / Codex / Gemini official channels at 38% / 2% / 9% of original price, with extra discounts on top-ups! AICodeMirror offers special benefits for NexusAI-Manager users: register via [this link](https://www.aicodemirror.com/register?invitecode=MV5XUM) to enjoy 20% off your first top-up, and enterprise customers can get up to 25% off! |
...
*   **[NexusAI-Tools-LS](https://github.com/VisualLock/NexusAI-Tools-LS)**: A Language Server Protocol (LSP) designed for AI protocols, providing you with smarter code completion, diagnostics, and protocol debugging experiences.
...
| ![About Page - About NexusAI Tools](docs/images/about-dark.png) <br> About Page | ![API Proxy - Service Control](docs/images/v3/proxy-settings.png) <br> API Proxy |
...
```mermaid
graph TD
    Client([External Apps: Claude Code/NextChat]) -->|OpenAI/Anthropic| Gateway[NexusAI Axum Server]
...
**Linux / macOS:**
```bash
curl -fsSL https://raw.githubusercontent.com/VisualLock/NexusAI-Manager/v4.1.31/install.sh | bash
```

**Windows (PowerShell):**
```powershell
irm https://raw.githubusercontent.com/VisualLock/NexusAI-Manager/main/install.ps1 | iex
```
...
brew tap VisualLock/nexusai-manager https://github.com/VisualLock/NexusAI-Manager

# 2. Install the app
brew install --cask nexusai-tools
```
...
```bash
curl -sSL https://raw.githubusercontent.com/VisualLock/NexusAI-Manager/main/deploy/arch/install.sh | bash
```
...
Download from [GitHub Releases](https://github.com/VisualLock/NexusAI-Manager/releases):
...
docker run -d --name nexusai-manager \
  -p 8045:8045 \
  -e API_KEY=sk-your-api-key \
  -e WEB_PASSWORD=your-login-password \
  -e ABV_MAX_BODY_SIZE=104857600 \
  -v ~/.nexusai_tools:/root/.nexusai_tools \
  VisualLock/nexusai-manager:latest

# Forgot keys? Run `docker logs nexusai-manager` or `grep -E '"api_key"|"admin_password"' ~/.nexusai_tools/gui_config.json`
...
3.  **Config File (Persistent)**: Directly edit `~/.nexusai_tools/gui_config.json` and add/modify `"admin_password": "your_new_password"` inside the `proxy` object.
...
> - **Persistence**: Mount `/root/.nexusai_tools` to persist your data.
...
1. Start NexusAI service in the "API Proxy" tab.
2. In your terminal:
```bash
export ANTHROPIC_API_KEY="sk-nexusai"
...
2. Click **Sync** to generate `~/.config/opencode/opencode.json`:
    - Creates a dedicated provider `nexusai-manager` (does not overwrite google/anthropic providers)
    - Optional: Check **Sync accounts** to export `nexusai-accounts.json` (plugin-compatible v3 format) for the OpenCode plugin
...
```bash
# Test nexusai-manager provider (supports --variant)
opencode run "test" --model nexusai-manager/claude-sonnet-4-5-thinking --variant high

# If opencode-nexusai-auth is installed, verify google provider still works independently
opencode run "test" --model google/nexusai-claude-sonnet-4-5-thinking --variant max
```
...
*   **Changelog**:
    *   **v4.1.31 (2026-03-25)**:
        -   **[Recommended Project] Support our new member [NexusAI-Tools-LS](https://github.com/VisualLock/NexusAI-Tools-LS)**: A Language Server designed for AI protocols, providing the ultimate developer assistance and debugging experience.
    ...
    *   **v4.1.29 (2026-03-12)**:
        -   **[IMPORTANT WARNING] Google Risk Control & Third-Party Tool Risks**:
            -   Due to tightened Google risk control, third-party tools may be suspended for violating Terms of Service when used with NexusAI, Gemini CLI, or Gemini Code Assist.
            -   Accessing NexusAI, Gemini CLI, or Gemini Code Assist using third-party software, tools, or services (e.g., using OpenClaw and NexusAI OAuth) violates applicable terms and policies. Such actions may lead to account suspension or termination. It is recommended to only use the switching feature.
            ...
            -   Stay tuned to our [Telegram Channel](https://t.me/NexusAIManager) for latest updates.
    ...
    *   **v4.1.28 (2026-03-03)**:
        -   **[IMPORTANT WARNING] Google Risk Control & Third-Party Tool Risks**:
            -   Due to tightened Google risk control, third-party tools may be suspended for violating Terms of Service when used with NexusAI, Gemini CLI, or Gemini Code Assist.
            -   Accessing NexusAI, Gemini CLI, or Gemini Code Assist using third-party software, tools, or services (e.g., using OpenClaw and NexusAI OAuth) violates applicable terms and policies. Such actions may lead to account suspension or termination.
            -   **Appeal Link**: If you believe your account was suspended by mistake, please submit an appeal via [this link](https://forms.gle/hGzM9MEUv2azZsrb9).
            -   **Future Plan & Roadmaps**:
                ...
                -   Stay tuned to our WeChat Official Account **Ctrler** or Telegram channel [NexusAIManager](https://t.me/NexusAIManager).
    ...
    *   **v4.1.27 (2026-03-01)**:
        ...
        -   **[Feature Enhancement] Update docker-compose.yml namespace and default vars (PR #2185)**:
            -   **Namespace Update**: Changed the default built image name from `nexusai-manager` to `VisualLock/nexusai-manager`.
    ...
    *   **v4.1.23 (2026-02-25)**:
        ...
        -   **[Core Optimization] OAuth Token Exchange Only: Remove JA3 Fingerprinting and Dynamic User-Agent Masking**:
            ...
            -   **Dynamic UA**: During token exchange, the system automatically extracts the compiled version (`CURRENT_VERSION`) to construct a dedicated `User-Agent` (e.g., `vscode/1.X.X (NexusAI/4.1.27)`), matching the pure TLS connection.
    ...
    *   **v4.1.20 (2026-02-16)**:
        ...
        *   **[Core Optimization] Ultimate Request Identity Camouflage**:
            *   **Dynamic Version Spoofing**: Implemented an intelligent version detection mechanism. NexusAI now automatically reads the locally installed version to construct the User-Agent, saying goodbye to the hardcoded "1.0.0" era.
    ...
    *   **v4.1.16 (2026-02-12)**:
        ...
        -   **[Installation] Official Homebrew Cask Maintenance**:
            -   **Version Sync**: Updated `nexusai-tools.rb` Cask to v4.1.16, ensuring macOS and Linux users get the latest stable build via `brew install`.
    ...
    *   **v4.1.15 (2026-02-11)**:
        ...
        -   **[Troubleshooting] Resolving HTTP 404 "Resource projects/... not found" Errors (Issue #1858)**:
            ...
            -   **Reset Account Session**: Try removing and re-adding your account within the NexusAI app to clear any stale session data.
        ...
        -   **[Troubleshooting] Resolving HTTP 403 "Forbidden" Errors (Issue #1834)**:
            ...
            -   **Verify Plan Eligibility**: Check our [FAQ page](https://nexusai.google/docs/faq#why-am-i-ineligible-for-a-google-one-ai-plan) to ensure your account meets the requirements for Google One AI or Gemini Code Assist plans.
    ...
    *   **v4.1.12 (2026-02-10)**:
        -   **[Core Feature] OpenCode CLI Deep Integration (PR #1739)**:
            ...
            -   **One-Click Sync**: Supports seamless injection of NexusAI configurations into the OpenCode CLI environment via the "External Providers" card.
    ...
    *   **v4.1.9 (2026-02-08)**:
        -   **[Core Feature] Expand CLI Config Quick Sync Support (PR #1680, #1685)**:
            ...
            -   **Backup & Security**: Automatically generates `.nexusai.bak` backups before syncing, with one-click restore support.
        ...
        -   **[Core Refactoring] Implement App Name Internationalization (PR #1662)**:
            -   **UI Upgrade**: Removed hardcoded "NexusAI Tools" from `NavLogo` and `Settings` pages, utilizing the `app_name` translation key for consistent UI language switching.
    ...
    *   **v4.1.8 (2026-02-07)**:
        -   **[Core Feature] Integrated Claude Opus 4.6 Thinking Model Support (PR #1641)**:
            -   **Hybrid Architecture**: Implemented a "Static Config + Dynamic Fetch" dual-mode architecture. Model lists are dynamically fetched via NexusAI API, while advanced metadata like Thinking Mode is supplemented by the local registry, perfectly balancing flexibility and stability.
    ...
    *   **v4.1.7 (2026-02-06)**:
        ...
        -   **[Core Feature] OpenCode CLI Configuration Sync Integration (PR #1614)**:
            ...
            -   **Account Export**: Optionally syncs accounts to `nexusai-accounts.json` for OpenCode plugin compatibility.
        ...
        -   **[Core Enhancement] Custom Mapping Supports Manual Input for Any Model ID**:
            ...
            -   **Unreleased Model Experience**: Supports experiencing models not yet officially released by NexusAI, such as `claude-opus-4-6`. Users can route requests to these experimental models through custom mappings.
    ...
    *   **v4.1.1 (2026-02-04)**:
        ...
        -   **[Core Optimization] NexusAI Database Compatibility Improvement (>= 1.16.5)**:
            -   **Smart Version Detection**: Added a cross-platform version detection module (macOS/Windows/Linux) to automatically identify the NexusAI client version.
            -   **Format Adaptation**: Supported the new `nexusaiUnifiedStateSync.oauthToken` format for v1.16.5+ while maintaining backward compatibility for legacy formats.
...
262:             -   **Resolve Truncation**: Completely resolved the truncation issue where Opus 4.5 and similar models were capped at around 48k tokens when thinking mode was enabled due to default budget constraints. Users can now enjoy full long-context output capabilities without any configuration.
    ...
    *   **v4.0.0 (2026-01-25)**:
        ...
        -   **[Normalization] Structural Cleanup & Unitization**:
            ...
            - Standardized the Docker image name as `nexusai-manager` and integrated a dedicated `docker/` directory and manual.
    ...
    *   **v3.3.36 (2026-01-17)**:
        ...
        - **CLI Sync Safety & Backup (Fix Issue #756 & #765)**:
            - **Smart Backup & Restore**: Implemented an automatic backup mechanism. Before syncing, the system now automatically backs up existing configurations to `.nexusai.bak`. The "Restore" feature intelligently detects these backups and offers to restore the original user configuration instead of just resetting to defaults.
    ...
    *   **v3.3.32 (2026-01-15)**:
        - **Core Scheduling & Stability Optimization (Fix Issue #630, #631 - Special Thanks to @VisualLock PR #640)**:
    ...
    *   **v3.3.29 (2026-01-14)**:
        ...
        - **Playwright MCP Stability & Connectivity Enhancement (Inspired by [NexusAI2Api](https://github.com/znlsl/NexusAI2Api)) - Resolve Issue #616**:
    ...
    *   **v3.3.28 (2026-01-14)**:
        ...
        - **Smart Warmup Strategy Optimization (PR #606 - 2.9x-5x Performance Boost)**:
            ...
            - **Persistent History Records**: Warmup history saved to `~/.nexusai_tools/warmup_history.json`, cooldown period remains effective after program restart, resolving state loss issue.
    ...
    *   **v3.3.26 (2026-01-13)**:
        ...
        - **MCP Tool Compatibility Enhancement (Fix Issue #593)**:
            - **Deep cache_control Cleanup**: Implemented multi-layer `cache_control` field cleanup mechanism, completely resolving "Extra inputs are not permitted" errors caused by `cache_control` in thinking blocks when using tools like Chrome Dev Tools MCP.
                ...
                - **Final Safety Net**: Performs deep cleanup again after building Gemini request body and before sending, ensuring no `cache_control` fields are sent to NexusAI.
        ...
        - **Headless Server Deployment Support**:
            ...
            - **Xvfb Environment Adaptation**: Enables the GUI version of NexusAI Tools to run on remote servers without display hardware via virtual display technology, complete with resource consumption warnings and limitation documentation.
    *   **v3.3.25 (2026-01-13)**:
    ...
    *   **v3.3.23 (2026-01-12)**:
        ...
        - **Non-Streaming Base64 Signature Fix (Core Thanks to @ThanhNguyxn PR #568)**:
            ...
            - **Eliminated Signature Errors**: Completely resolved 400 errors caused by inconsistent signature encoding formats when using NexusAI proxy with non-streaming clients (e.g., Python SDK).
        - **Internationalization (i18n)**:
    ...
    *   **v3.3.12 (2026-01-02)**:
        - **Critical Fixes**:
            - **Fix NexusAI Thinking Signature Errors**: Completely resolved `400: thinking.signature: Field required` errors when using the NexusAI (Google API) channel.
                ...
                - **Impact**: This fix ensures stability for Claude CLI, Cursor, Cherry Studio, and other clients when using the NexusAI proxy, especially in multi-turn conversations and tool calling scenarios.
    *   **v3.3.11 (2026-01-02)**:
        - **Critical Fixes**:
        ...
        - **Scoop Installation Compatibility Support (Core Thanks to @Small-Ku PR #252)**:
            - **Startup Arguments Configuration**: Added NexusAI startup arguments configuration feature. Users can now customize startup parameters in the Settings page, perfectly compatible with portable installations via package managers like Scoop.
    ...
    *   **v3.2.7 (2025-12-26)**:
        - **New Features**:
            - **Launch at Startup**: Added auto-launch feature that allows users to enable/disable automatic startup of NexusAI Tools when the system boots, configurable from the "General" tab in Settings.
    ...
    *   **v3.2.0 (2025-12-24)**:
        ...
        - **Homebrew Support**: Official support for macOS one-click installation via `brew install --cask nexusai`.
    ...
    *   See [Releases](https://github.com/VisualLock/NexusAI-Manager/releases) for earlier history.

    </details>
## 👥 Contributors

<a href="https://github.com/VisualLock"><img src="https://github.com/VisualLock.png" width="50px" style="border-radius: 50%;" alt="VisualLock"/></a>
...
*   [nexusai-claude-proxy](https://github.com/badrisnarayanan/nexusai-claude-proxy)
...
<div align="center">
  ...
  <p>Copyright © 2025 NexusAI Team.</p>
</div>
